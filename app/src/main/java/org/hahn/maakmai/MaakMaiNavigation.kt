...
class MaakMaiNavigationActions(private val navController: NavController) {
    ...
    /**
     * Route that opens the Add screen as an in-place edit of an already-captured
     * shared bookmark. Only the bookmark id is passed — the URL is read from the
    ...
    fun shareCaptureRoute(bookmarkId: UUID): String {
        return "$ADD_EDIT_BOOKMARK_SCREEN/Add Bookmark?$BOOKMARK_ID_ARG=$bookmarkId"
    }
}